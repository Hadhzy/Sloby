...
export class SlobySet {
  ...
  add(addedValues: ExpectedSetValues) {
    ...
    if (isTheValueWronglyDeclered)
      throw new Error(
        `If you are trying to add multiple values into your set, make sure that you add at least two values into the Array, otherwise add just a single value without arrays`
      )
    const areTheValuesMultiple =
      Array.isArray(addedValues) && addedValues.length > 1
    ...
    if (!areTheValuesMultiple) {
      ...
      if (alreadyIncluded)
        throw new Error(
          'The list of values that you trying to add, some of them are already in the set'
        )
      return addedValues.forEach((value: ExpectedSetValues) =>
  ...
  private multipleElementCheckker(element_list: ExpectedSetValues[]) {
     ...
     */
    const elementsAreIncluded = element_list.some(
      (element: ExpectedSetValues) => this.values.includes(element)
    )
