...
export default function BluePrintObject(attributes: TAttributes, w:string = "w-screen", h:string = "h-screen"){
  ...
  function startDrag(event: any){
    ...
    return (
        <motion.div drag = {true }  dragMomentum={false} onPointerDown={startDrag} dragControls={controls} className={`${h} ${w} w-max h-max text-white...`}>
        <div className="flex w-36 flex-col rounded-xl bg-dark-darker">

        <p className="to-bg-dark-border rounded-t-xl bg-gradient-to-r from-indigo-500 px-2 py-1">{name}</p>
